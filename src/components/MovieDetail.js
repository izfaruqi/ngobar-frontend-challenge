        ...
        <Transition visible={props.movieDetail.isOpen} animation="scale" duration={200}>
            <Modal dimmer="blurring" open={props.movieDetail.isOpen} centered={false} onClose={() => props.dispatch(setMovieDetailVisibilty(false))}>
                ...
                <Container style={{position: 'relative', zIndex: 2}}>
                    <Image style={{width: 150, objectFit: 'cover', position: 'absolute', zIndex: 2, bottom: -90, left: 40, boxShadow: "10px 15px 15px -10px rgba(0,0,0,0.5)", borderRadius: 5}} src={ "https://image.tmdb.org/t/p/w500" + props.movieDetail.movie.poster_path}></Image>
                    <Image style={{width: "100%", height: 200, objectFit: 'cover'}} src={ "https://image.tmdb.org/t/p/w1280" + props.movieDetail.movie.backdrop_path}></Image>
                </Container>
                ...
                <Modal.Content>
                    <Container style={{ paddingLeft: 200, marginBottom: 15}}>
                        ...
                        <div style={{marginBottom: 10}}>{props.movieDetail.movie.runtime + " m (" + (Math.floor(props.movieDetail.movie.runtime/60.0) + " h and " + (Math.floor(props.movieDetail.movie.runtime % 60.0) + " m)"))}</div>
                        <div style={{display: 'flex', flexDirection: 'row', flexWrap: 'wrap'}}>
                            {props.movieDetail.movie.production_companies && props.movieDetail.movie.production_companies.map(company => <Label color="white" image style={{display: 'inline-block', marginBottom: 4, marginRight: 4, marginLeft: 0}}>
                                {company.logo_path && <Image style={{padding: "4px 0px 3px 3px"}} src={"https://image.tmdb.org/t/p/w45" + company.logo_path}></Image>}
                                {company.name}
                            </Label>)}
                        </div>
                        <Segment>
